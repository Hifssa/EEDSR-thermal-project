...
# Training function
def train_model(model, model_name, dataloader, num_epochs=5, lr=1e-4):
...
if os.path.exists(srcnn_model_path):
    ...
    # Load SRCNN training losses if available
    srcnn_losses = []
    if os.path.exists('model_evaluation_results.json'):
        try:
            ...
            print("Could not load SRCNN training losses from previous run")
else:
    print("SRCNN model not found. Please train it first.")
    exit()

# Check if EDSR model already exists
...
if os.path.exists(edsr_model_path):
    ...
    # Load EDSR training losses if available
    edsr_losses = []
    if os.path.exists('model_evaluation_results.json'):
        try:
            with open('model_evaluation_results.json', 'r') as f:
            ...
            print("Could not load EDSR training losses from previous run")
else:
    # Train EDSR model if not found
    print("EDSR model not found. Training EDSR model...")
