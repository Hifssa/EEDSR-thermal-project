...
# Create directories
os.makedirs('swinir_results/error_maps', exist_ok=True)
os.makedirs('swinir_results/comparisons', exist_ok=True)
os.makedirs('swinir_results/metrics', exist_ok=True)
os.makedirs('swinir_results/models', exist_ok=True)
...
def train_swinir(model, train_loader, val_loader, epochs=50, lr=1e-4):
    ...
    for epoch in range(1, epochs+1):
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), f"swinir_results/models/swinir_epoch_{epoch}.pth")

    torch.save(model.state_dict(), "swinir_results/models/swinir_final.pth")
    return model
        ...
        for lr_imgs, hr_imgs, img_names in test_loader:
            ...
            for j in range(sr_imgs.shape[0]):
                ...
                create_error_map(hr_imgs[j:j+1], sr_imgs[j:j+1],
                                 ...
                                 filename=f"swinir_results/error_maps/{img_names[j]}_error.png")
                create_comparison_figure(lr_imgs[j:j+1], sr_imgs[j:j+1], hr_imgs[j:j+1],
                                         filename=f"swinir_results/comparisons/{img_names[j]}_compare.png",
                                         sample_name=img_names[j],
                                         psnr=psnr_val, ssim=ssim_val)
    with open("swinir_results/metrics/metrics.json", "w") as f:
...
# 5. Run Everything
# ==========================
dataset_path = "Ir"  # <- Your dataset folder
