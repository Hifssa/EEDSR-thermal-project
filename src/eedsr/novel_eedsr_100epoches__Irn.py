...
class InfraredDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return lr, hr, img_path  # Return path for tracking
...
def show_results(lr, sr, hr, epoch, result_prefix, image_info=""):
    ...
    print(f"Saved detailed results for epoch {epoch}: {image_info}")

def train_model(train_loader, test_loader, scale=4, epochs=100, result_prefix="Irn_EEDSR"):
    ...
    # Learning rate scheduler - more frequent steps for 100 epochs
    ...
    # Track shown images to ensure diversity
    shown_indices = set()
    test_dataset = test_loader.dataset
    
    print(f"Starting training for {epochs} epochs with result prefix: {result_prefix}")
    
    for epoch in range(1, epochs+1):
        model.train()
        epoch_loss = 0
        for i, (lr, hr, paths) in enumerate(train_loader):
            ...
            # Print progress
            if i % 50 == 0:
                print(f"Epoch {epoch:03d}/{epochs}, Batch {i:03d}, Loss: {current_loss:.4f}")
        ...
        psnr_vals, ssim_vals = [], []
        with torch.no_grad():
            for i, (lr, hr, paths) in enumerate(test_loader):
                lr, hr = lr.to(device), hr.to(device)
                sr = model(lr)
                ...
                # Show results every 10 epochs with DIFFERENT images
                if i == 0 and epoch % 10 == 0:
                    # Find a batch that hasn't been shown much
                    batch_to_show = i
                    image_idx_in_batch = 0
                    
                    # Get image info for filename
                    image_name = os.path.basename(paths[image_idx_in_batch])
                    image_info = f"Image: {image_name}"
                    
                    print(f"Visualizing epoch {epoch} with image: {image_name}")
                    show_results(lr[image_idx_in_batch], sr[image_idx_in_batch], hr[image_idx_in_batch], 
                               epoch, result_prefix, image_info)
        ...
        print(f"Epoch {epoch:03d}: Loss={avg_loss:.4f}, PSNR={avg_psnr:.4f}, SSIM={avg_ssim:.4f}, LR={scheduler.get_last_lr()[0]:.2e}")
        
        # Save model checkpoint every 25 epochs
        if epoch % 25 == 0:
            checkpoint_path = f"{result_prefix}_epoch_{epoch:03d}.pth"
        ...
        if epoch % 10 == 0:
            ...
            plt.suptitle(f"Training Progress - {result_prefix} (Epoch {epoch})", fontsize=16, fontweight='bold')
            plt.tight_layout()
            
            progress_path = f"{result_prefix}_progress_epoch_{epoch:03d}.png"
            plt.savefig(progress_path, dpi=300, bbox_inches='tight')
            plt.close()
            print(f"Saved progress plot: {progress_path}")
    ...
    plt.suptitle(f"Final Training History - {result_prefix}", fontsize=16, fontweight='bold')
    plt.tight_layout()
    
    history_path = f"{result_prefix}_training_history.png"
    plt.savefig(history_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved final training history: {history_path}")
...
if __name__ == "__main__":
    ...
    result_prefix = f"Irn_EEDSR_100epochs_{timestamp}"
    
    print(f"Starting training for 100 epochs with result prefix: {result_prefix}")
    
    # Train for 100 epochs
    model, train_losses, val_psnr, val_ssim = train_model(
        train_loader, test_loader, scale=4, epochs=100, result_prefix=result_prefix
    )
    ...
    print(f"Best PSNR: {np.max(val_psnr):.4f} dB at epoch {np.argmax(val_psnr) + 1}")
    print(f"Best SSIM: {np.max(val_ssim):.4f} at epoch {np.argmax(val_ssim) + 1}")
    print(f"All models and results saved with prefix: {result_prefix}")
    print("="*60)