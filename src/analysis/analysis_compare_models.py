...
os.makedirs('analysis_results/error_maps', exist_ok=True)
os.makedirs('analysis_results/comparisons', exist_ok=True)
os.makedirs('analysis_results/metrics', exist_ok=True)
...
# ============================================
# 1. Dataset Loader (Same as before)
# ============================================
class M3FDDataset:
...
def plot_metrics_comparison(avg_results, filename):
    ...
    ax1.bar(x_pos, psnr_vals, yerr=psnr_err, align='center', alpha=0.7, capsize=10)
    ...
    # SSIM comparison
    ax2.bar(x_pos, ssim_vals, yerr=ssim_err, align='center', alpha=0.7, capsize=10)
    ax2.set_ylabel('SSIM')
...
def main():
    ...
    data_loader = DataLoader(dataset, batch_size=1, shuffle=False)  # Batch size 1 for individual image processing
    ...
    avg_results, all_results, sample_results = evaluate_models(models_dict, data_loader, device, num_samples=5)
    ...
    with open('analysis_results/metrics/evaluation_results.json', 'w') as f:
        json.dump({
    ...
    plot_metrics_comparison(avg_results, 'analysis_results/metrics/model_comparison.png')
    
    # Create visualizations for sample images
    print("\nCreating visualizations for sample images...")
    for i, sample in enumerate(sample_results):
        ...
        create_comparison_figure(
            lr, sr_dict, hr, 
            f'analysis_results/comparisons/sample_{i+1}_{img_name}.png',
            f'Sample {i+1} ({img_name})'
        )
        ...
        for model_name, sr in sr_dict.items():
            create_error_map(
                ...
                f"analysis_results/error_maps/{model_name}_sample_{i+1}_{img_name}.png"
            )
    
    print("\nAnalysis complete! Results saved in the 'analysis_results' directory.")
    print("Files created:")
    print("- analysis_results/metrics/evaluation_results.json (detailed metrics)")
    print("- analysis_results/metrics/model_comparison.png (metrics comparison chart)")
    print("- analysis_results/comparisons/ (comparison figures for sample images)")
    print("- analysis_results/error_maps/ (individual error maps)")
