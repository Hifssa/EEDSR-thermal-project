...
# Training function
def train_model(model, model_name, dataloader, num_epochs=50, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (lr_batch, hr_batch) in enumerate(dataloader):
            ...
            loss = criterion(sr_batch, hr_batch) / accumulation_steps
            loss.backward()
            ...
            if i % 20 == 0:
                print(f"{model_name} Epoch {epoch+1}/{num_epochs}, Batch {i}, Loss: {loss.item() * accumulation_steps:.4f}")
        
        scheduler.step()
        avg_loss = total_loss / len(dataloader)
        train_losses.append(avg_loss)
        
        print(f"{model_name} Epoch {epoch+1}/{num_epochs}, Average Loss: {avg_loss:.4f}, LR: {scheduler.get_last_lr()[0]:.2e}")
    
    training_time = time.time() - start_time
    print(f"{model_name} training completed in {training_time/60:.2f} minutes")
    
    # Save the trained model
    model_path = f'{model_name.lower()}_model_{num_epochs}epochs.pth'
...
# Train SRCNN model
srcnn_losses, srcnn_time = train_model(srcnn_model, "SRCNN", dataloader, num_epochs=50)

# Train EDSR model
edsr_losses, edsr_time = train_model(edsr_model, "EDSR", dataloader, num_epochs=50)

# Plot training losses
plt.figure(figsize=(10, 6))
plt.plot(srcnn_losses, label='SRCNN')
plt.plot(edsr_losses, label='EDSR')
plt.xlabel('Epoch')
plt.ylabel('Loss')
...
results = {
    "SRCNN": {
        ...
        "Training_time_minutes": float(srcnn_time / 60),
    ...
    "EDSR": {
        ...
        "Training_time_minutes": float(edsr_time / 60),
...
def visualize_comparison(srcnn_model, edsr_model, dataset, num_samples=3):
    for sample_idx in range(num_samples):
        ...
        # Error maps
        create_error_map(hr_img, srcnn_output, "SRCNN", None)
...
print(f"SRCNN Training Time: {srcnn_time/60:.2f} minutes")
print(f"EDSR Training Time: {edsr_time/60:.2f} minutes")
...
print("Files created:")
print("- srcnn_model_50epochs.pth (SRCNN model weights)")
print("- edsr_model_50epochs.pth (EDSR model weights)")
print("- model_evaluation_results.json (evaluation metrics)")
print("- training_loss_comparison.png (training curves)")
